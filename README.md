...
lh stats --by-kind # kind ごとの件数・pass率・平均 duration・gate.status
```
...
{"tasks":[
  {"id":"docs-sync","kind":"docs","check":"cd docs && bun test","prompt":"docs/README.md を docs/src/cli.ts の実装に同期。src/ と test/ は変更禁止。"},
...
各タスクは独立の `id`/`kind`/`check` を持ち、タスク毎に check+修復ループが走る(タスク間で文脈はリセット)。部分失敗は次の独立タスクへ続行し、全タスク完了後に**再検証スイープ**(通過済み check の再実行)で兄弟タスクの bash 副作用による巻き戻しを検出する。`--max-time` は**バッチ全体**の総実時間予算(予算切れの未着手タスクは `not_run`)。進行状況はタスク完了毎にセッションへ逐次保存されるため、呼び出し元が途中で死んでも完了済みタスクの成果・check 結果は残る。
...
- **委譲前に実績を見る(stats 事前チェック)**: 委譲判断の前に `lh stats --by-kind --json` を読み、投げようとしている種類(`--kind`)の `gate.status` が `"block"` なら委譲しない——自力実行するか、作業指示を具体化してから再挑戦する。現行ゲートは graded ≥ 3 かつ pass `rate` < 50(fail が過半)で block。distill/scout にも同じ gate を適用する。
...
- **前処理は委譲と損益構造が違う**: `lh distill` / `lh scout` は作業を任せるのではなく、大きな入力や探索ファンアウトをローカルで処理し、引用付き digest だけを上位エージェントに返す。節約は固定床ではなく入力サイズ・探索ファイル数に比例し、削った文脈は残りターンでも再課金されない。大きいログ/ファイルほど `distill`、読む場所が不明な所在調査ほど `scout` が効きやすいが、citation の recall は評価で測る必要があるため、digest だけで編集せず cited range を読み直す。
...
- **委譲前に stats を確認させる**: `lh stats --by-kind --json` を読み、当該 kind の `gate.status` が `"block"` なら委譲せず自力実行 or 作業指示を具体化。前処理では `distill` / `scout` kind に同じ gate を使う。

CLAUDE.md 用スニペット(コピペ可):

```text
ローカル委譲CLI `lh` が使える(delegate-local スキル)。委譲/前処理前に `lh stats --by-kind --json` を見て、当該 kind の `gate.status` が `"block"` なら使わない。多ターンで機械的、かつ厳密に検証可能なタスク(大規模リネーム・ボイラープレート・大きなテスト作成; 目安 baseline $0.15 超)のみ委譲する。小さいタスクは自分でやる方が安い(委譲は固定オーバーヘッドがある)。単発は heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200` を Bash タイムアウト 1500000 ms で(同期。submit/wait は headless では使わない)。**独立した委譲向きタスクが複数あるなら 1件ずつ `lh -p` を呼ばず、`lh batch --tasks - --json --cwd <絶対パス> --max-time 1800` に JSON マニフェスト(各タスクに id/kind/check/prompt)を heredoc で渡して1コールに束ねる**。1000行または64KBを超える既知入力は `lh distill -q`、5ファイル以上読む見込みの所在調査は `lh scout -q` を検討する。返答後は各タスクの `check.exit_code===0` と `report.changed_files`/diff、前処理なら cited range を確認し、`lh feedback <session_id> [--task <id>] pass|fail --source claude-code --notes "<理由>"` を必ず記録する。委譲は壁時計を3〜7倍にする取引である点に注意。
```

AGENTS.md 用スニペット(コピペ可):

```text
ローカル委譲CLI `lh`(Qwen 3.6 27B / Ollama)が使える。委譲/前処理前に `lh stats --by-kind --json` を見て当該 kind の `gate.status` が `"block"` なら使わない。多ターンで機械的、かつ厳密に検証可能なタスクのみ委譲する(委譲には固定オーバーヘッドがあり、数ターンで終わるタスクは自分でやる方が安い)。単発は heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200`、Bash タイムアウトは ≥900000 ms(同期。submit/wait は headless では使わない)。独立した委譲向きが複数あるなら `lh batch --tasks - --json --cwd <絶対パス> --max-time 1800` に JSON マニフェスト(各タスクに id/kind/check/prompt)を渡して1コールに束ねる。1000行または64KBを超える既知入力は `lh distill -q`、5ファイル以上読む見込みの所在調査は `lh scout -q` を検討する。返答後は各タスクの `check.exit_code===0` と `report.changed_files`/diff、前処理なら cited range を確認し、`lh feedback <session_id> [--task <id>] pass|fail --source codex --notes "<検証内容/失敗理由>"` を必ず記録。詳細は integrations/codex/AGENTS-snippet.md。
```
