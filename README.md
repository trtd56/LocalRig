...
- **委譲前に実績を見る(stats 事前チェック)**: 委譲判断の前に `lh stats --by-kind --json` を読み、投げようとしている種類(`--kind`)が graded ≥ 3 件あって pass `rate` が 50 未満(fail が過半)なら委譲しない——自力実行するか、作業指示を具体化してから再挑戦する。書きっぱなしだった feedback を委譲判断に接続する learning loop(効果の定量測定はデータ蓄積後の宿題)。
...
- **委譲前に stats を確認させる**: `lh stats --by-kind --json` を読み、当該 kind が graded ≥ 3 かつ pass rate < 50 なら委譲せず自力実行 or 作業指示を具体化。

CLAUDE.md 用スニペット(コピペ可):

```text
ローカル委譲CLI `lh` が使える(delegate-local スキル)。委譲前に `lh stats --by-kind --json` を見て、当該 kind が graded≥3 かつ pass rate<50 なら委譲しない。多ターンで機械的、かつ厳密に検証可能なタスク(大規模リネーム・ボイラープレート・大きなテスト作成; 目安 baseline $0.15 超)のみ委譲する。小さいタスクは自分でやる方が安い(委譲は固定オーバーヘッドがある)。単発は heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200` を Bash タイムアウト 1500000 ms で(同期。submit/wait は headless では使わない)。**独立した委譲向きタスクが複数あるなら 1件ずつ `lh -p` を呼ばず、`lh batch --tasks - --json --cwd <絶対パス> --max-time 1800` に JSON マニフェスト(各タスクに id/kind/check/prompt)を heredoc で渡して1コールに束ねる**(手組みで並べるより約3割安く、タスク単位で check・修復・永続化される)。返答後は各タスクの `check.exit_code===0` と `report.changed_files`/diff を確認し、`lh feedback <session_id> [--task <id>] pass|fail --source claude-code --notes "<理由>"` を必ず記録する。委譲は壁時計を3〜7倍にする取引である点に注意。
```

AGENTS.md 用スニペット(コピペ可):

```text
ローカル委譲CLI `lh`(Qwen 3.6 27B / Ollama)が使える。委譲前に `lh stats --by-kind --json` を見て当該 kind が graded≥3 かつ pass rate<50 なら委譲しない。多ターンで機械的、かつ厳密に検証可能なタスクのみ委譲する(委譲には固定オーバーヘッドがあり、数ターンで終わるタスクは自分でやる方が安い)。単発は heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200`、Bash タイムアウトは ≥900000 ms(同期。submit/wait は headless では使わない)。独立した委譲向きが複数あるなら `lh batch --tasks - --json --cwd <絶対パス> --max-time 1800` に JSON マニフェスト(各タスクに id/kind/check/prompt)を渡して1コールに束ねる(手組みで `lh -p` を並べるより安く、タスク単位で check・修復・永続化される)。返答後は各タスクの `check.exit_code===0` と `report.changed_files`/diff を確認し、`lh feedback <session_id> [--task <id>] pass|fail --source codex --notes "<検証内容/失敗理由>"` を必ず記録。詳細は integrations/codex/AGENTS-snippet.md。
```
