...
大きいタスクでは呼び出し側の待ち時間を塞がないために detached 実行を使える:
...
- **実時間トレードオフ**: 委譲アーム全体で壁時計 **約3.7〜4.1倍**(`--check` 実行分が上乗せ)。API コストを壁時計時間(約3〜7倍)と引き換える取引。
