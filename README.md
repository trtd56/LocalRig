...
- **委譲前に実績を見る(stats 事前チェック)**: 委譲判断の前に `lh stats --by-kind --json` を読み、投げようとしている種類(`--kind`)の `gate.status` が `"block"` なら委譲しない——自力実行するか、作業指示を具体化してから再挑戦する。現行ゲートは graded ≥ 3 かつ pass `rate` < 50(fail が過半)で block。distill/scout/research にも同じ gate を適用する。
