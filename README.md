...
- **委譲がたまったらバッチにさせる**: 固定費の大半はセッション起動費なので、独立した委譲向きタスクが複数あるなら1セッションでまとめて委譲させる(1件あたり $0.06〜0.08 まで下がる)。
...
```text
ローカル委譲CLI `lh` が使える(delegate-local スキル)。委譲前に `lh stats --by-kind --json` を見て、当該 kind が graded≥3 かつ pass rate<50 なら委譲しない。多ターンで機械的、かつ厳密に検証可能なタスク(大規模リネーム・ボイラープレート・大きなテスト作成; 目安 baseline $0.15 超)のみ委譲する。小さいタスクは自分でやる方が安い(委譲は約$0.11〜0.18の固定オーバーヘッド)が、独立した委譲向きが複数あるなら1セッションでまとめて委譲すると1件あたり $0.06〜0.08 に下がる。実行: heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200` を Bash タイムアウト 1500000 ms で(単一タスクは同期。submit/wait は headless では使わない)。返答後は `check.exit_code===0` と `report.changed_files`/diff を確認し、`lh feedback <session_id> pass|fail --source claude-code --notes "<理由>"` を必ず記録する。委譲は壁時計を3〜7倍にする取引である点に注意。
```

AGENTS.md 用スニペット(コピペ可):

```text
ローカル委譲CLI `lh`(Qwen 3.6 27B / Ollama)が使える。委譲前に `lh stats --by-kind --json` を見て当該 kind が graded≥3 かつ pass rate<50 なら委譲しない。多ターンで機械的、かつ厳密に検証可能なタスクのみ委譲する(委譲は約$0.11〜0.18の固定コストがあり、数ターンで終わるタスクは自分でやる方が安い; 損益分岐 baseline $0.15)。独立した委譲向きが複数あるなら1セッションでまとめて委譲すると1件あたり $0.06〜0.08 に下がる。実行: heredoc で `lh -p - --json --cwd <絶対パス> --kind <種類> --check "<受け入れコマンド>" --max-time 1200`、Bash タイムアウトは ≥900000 ms(単一タスクは同期。submit/wait は headless では使わない)。返答後は `check.exit_code===0` と `report.changed_files`/diff を確認し、`lh feedback <session_id> pass|fail --source codex --notes "<検証内容/失敗理由>"` を必ず記録。詳細は integrations/codex/AGENTS-snippet.md。
```
