...
- 許可モデル: デフォルトは write/edit/bash を確認、`--yolo` で自動承認
...
├── config.ts         # 設定(model, num_ctx, temperature, limits)
...
| num_ctx | ≥16k必須(4kだとスキーマが切れてtool calling が静かに壊れる)、本機は65536 | OpenCode docs |
