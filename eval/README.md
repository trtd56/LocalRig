...
### 評価専用 Ollama daemon と干渉監視

速度・品質ゲートは共有GPU競合を避けるため専用daemonを使う。`OLLAMA_HOST`はdaemon起動側だけでなく、`eval/run.ts`を起動する同じシェルにもexportする。

```sh
bun run eval:daemon -- start --port 11500
export OLLAMA_HOST=http://127.0.0.1:11500
bun run eval:daemon -- status --port 11500

# 実験セルごとに再起動して指定する
bun run eval:daemon -- stop --port 11500
bun run eval:daemon -- start --port 11500 --flash-attention --kv-cache-type q8_0

# 手動watcher（eval runner自身も30秒間隔で自動記録する）
bun run eval:watch-daemon -- --url "$OLLAMA_HOST" --out eval/results/daemon-watch/manual.jsonl

bun run eval:daemon -- stop --port 11500
```

daemonは既定で`OLLAMA_NUM_PARALLEL=1`、`OLLAMA_MAX_LOADED_MODELS=1`、`OLLAMA_KEEP_ALIVE=30m`。PID/logは`$LH_HOME/daemon/<port>/`に置く。eval runnerはtask前後の`/api/ps`と連続watcherを保存し、実験対象外digestがsample区間に現れた結果を`eval/results/envfail/*.envfail.json`へ退避して最大3回再走する。別daemonも同じGPUを使う環境では`LH_EVAL_WATCH_URLS=http://127.0.0.1:11434`（複数はカンマ区切り）を必ず指定し、評価専用daemon外のrunnerも門番に含める。評価接続先ではarm digestをallowlistし、追加監視先ではrunnerが1体でも映れば外来扱いにするため、同digestの別workloadも見逃さない。
