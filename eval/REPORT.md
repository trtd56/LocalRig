...
#### 高速化キャンペーン実装基盤（2026-07-11）

Phase A/Bのopt-in基盤を実装した。harness同士の反復ゲートは`--skip-cost`でbilled cost欠損を明示的に除外できる。model turnにはserver total、queue残差、client overheadを追加し、one-shot/batch/eval task前後の`/api/ps`、30秒daemon watcher、外来digest検出時の`.envfail`退避と最大3回再走を追加した。専用daemon helperはport 11500、parallel/max-loaded各1、keep-alive 30mを既定とし、FA/KVセルを再起動で切り替えられる。

thinkingはone-shot/REPL/detached workerまで`--think`/`--no-think`が伝播し、`LH_THINK_BY_KIND`は空map既定のopt-in、`LH_NUM_PREDICT`も環境変数から読める。SessionRecordへeffective think、system prompt SHA-256、runner snapshotsをadditive記録する。ここまでで既定のthinking動作・モデル・server数値設定は変更していない。P0以降の実測値は専用daemonの分散go/no-go通過後に追記する。
