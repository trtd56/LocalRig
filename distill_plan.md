# distill_plan.md — 前処理レイヤー実装計画(P0 `lh distill` / P1 `lh scout` / P2 統合深化 / P3 `lh research`)
...
# P3: `lh research` — Web検索・取得前処理

進捗メモ(2026-07-08): `src/research.ts`、CLI、snapshot永続化、Brave/SearXNG/direct URL adapter、SSRF防御、モデル不要の固定fixture評価、`claude-research` armの足場まで実装済み。未実施なのはライブWeb+実モデルの品質測定、baselineとの同日n=3比較、黒字になる発火閾値の校正。

## 位置づけと設計

researchは「検索結果や複数ページの生本文を上位モデルへ入れる前」に、検索・取得・選別を分離するcontext compilerである。観点なしのgeneric summaryはrecallを検証できないため `-q/--query` を必須にする。処理境界は次の通り。

1. ローカルLLMは質問から1〜3個の検索queryを計画する。
2. ハーネスがBrave Search（`BRAVE_SEARCH_API_KEY`）、SearXNG（`LH_SEARXNG_URL` / `--search-url`）、またはdirect URLから候補を受け、URLをcanonicalize/dedupeする。
3. 標準fetcherがHTTP(S)以外、credentials、localhost、private/link-local/reserved addressを拒否し、DNSとredirectの各hopを再検査する。本文はuntrusted dataであり、ページ内の命令・role message・tool requestには従わない。
4. HTMLを正規化し、取得時本文をSHA-256付きsnapshotに固定してから、distillのmap-reduceとcitation verifierで質問に関係する根拠だけを抽出する。
5. CLIは `$LH_HOME/research/<session_id>/<sha256>.txt` と `manifest.json` を保存する。digest citationはURL/title/fetched_at/hash/offset/quoteを持ち、上位エージェントはdigestを地図としてsnapshot・鮮度・矛盾を確認する。
6. kind既定は`research`。利用後は `lh feedback` を記録し、`lh stats --by-kind` のgateへ実績を蓄積する。

CLI:

```sh
BRAVE_SEARCH_API_KEY=... lh research -q "2026年時点の仕様は?" --search-provider brave --max-results 8 --max-pages 5 --json
LH_SEARXNG_URL=https://search.example.org lh research -q "一次資料は?" --json
lh research -q "2資料の合意点と矛盾は?" https://example.com/a https://example.com/b --budget 2000 --json
```

## 決定的評価と残課題

`bun run eval:research-adapter` はライブWeb/Ollamaを使わず、固定HTTP fixtureとfake completionでcanonicalize、dedupe、HTML正規化、snapshot検証、not_found、prompt injection拒否、古い矛盾資料の非採用を測る。現時点の結果は citation precision=1、植込みcitation recall=1、32,784 input tokens→375 output tokens（圧縮率0.0114）、取得7ページ。これはadapterの回帰試験であり、実モデルの判断品質やライブWeb変動、上位モデルのコスト削減を示さない。

`eval/run.ts` には `claude-research` / `RESEARCH_NUDGE` とkind=research session・citation drop・compression・page count・quality failureの集計を追加した。ただしrunner中だけephemeral fixture serverを維持してURLをtaskへ渡すhookはまだなく、research用の実モデルfixtureとbaseline/arm同日n=3は未実施。発火条件は「複数ページの生本文が大きく、質問指向の意味的選別が必要」の定性的条件に留め、次回は同日・同CLI version・warm統制でn=3を取り、ページ数/総本文tokensの損益分岐を決める。
